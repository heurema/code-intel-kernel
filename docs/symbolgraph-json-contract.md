...
The eval harness report uses `eval_contract_version = "0.5"` after Phase 3B-A because it includes `symbol_cases`, `source_evidence_cases`, `source_context_cases`, `lsp_diagnostics_cases`, and adversarial refusal cases. The `symbols` JSON contract remains `0.1`.
